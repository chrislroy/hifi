...
inline QScriptValue convertScriptValue(QScriptEngine* e, const xColor& v) {
    return xColorToScriptValue(e, v);
}
...
inline glm::vec2 vec2_convertFromScriptValue(const QScriptValue& v, bool& isValid) {
    ...
    if (x.isValid() && y.isValid()) {
        ...
        isValid = !glm::isnan(newValue.x) && !glm::isnan(newValue.y);
        if (isValid) {
            return newValue;
        }
    }
...
inline glm::vec3 vec3_convertFromScriptValue(const QScriptValue& v, bool& isValid) {
    ...
    if (x.isValid() && y.isValid() && z.isValid()) {
        ...
        isValid = !glm::isnan(newValue.x) && !glm::isnan(newValue.y) && !glm::isnan(newValue.z);
        if (isValid) {
            return newValue;
        }
    }
